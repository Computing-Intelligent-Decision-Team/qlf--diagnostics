...
class DDPGAgent:
    ...
    def __init__(
        ...
        self.actor_optimizer = optim.Adam(     #优化器
    ...
    def update_model(self) -> torch.Tensor:
        ...
        state = torch.FloatTensor(samples["obs"]).to(device)  #抽取batchsize数量的数据
        next_state = torch.FloatTensor(samples["next_obs"]).to(device)
        action = torch.FloatTensor(samples["acts"]).to(device)
        reward = torch.FloatTensor(samples["rews"].reshape(-1, 1)).to(device)
        done = torch.FloatTensor(samples["done"].reshape(-1, 1)).to(device)
       #从经验池进行数据抽取，抽取一个batch_size大小，[state,next_state,action,reward,done]
        masks = 1 - done                                         
        next_action = self.actor_target(next_state) #将s’输入actor_target网络得到a’
        next_value = self.critic_target(next_state, next_action) #输入s’，a’得到Q’
        curr_return = reward + self.gamma * next_value * masks   #计算r+Q'
        values = self.critic(state, action)  #计算Q
        critic_loss = F.mse_loss(values, curr_return) #计算r+Q'与Q'之间的均方误差（即对r的预测偏差）
        self.critic_optimizer.zero_grad()  #清除梯度积累
        critic_loss.backward()  #反向传播
        self.critic_optimizer.step() #按学习率更新 critic 参数
        actor_loss = -self.critic(state, self.actor(state)).mean() #最小化负的 Q 值
    
        self.actor_optimizer.zero_grad() #actor梯度清零
        actor_loss.backward() #反向传播
        self.actor_optimizer.step() #更新actor参数
        self._target_soft_update() #软更新 Target 网络
    ...
    def train(self, num_steps: int, plotting_interval: int = 20000):
        ...
        for self.total_step in range(1, num_steps + 1): 
            ...
            if terminated or truncated:
                state, info = self.env.reset() #state是observation，每一行为一个器件相关参数
    ...
    def _target_soft_update(self): #软更新target网络
        """Soft-update: target = tau*local + (1-tau)*target."""
        tau = self.tau      #更新时参考0.005 actor网络参数
